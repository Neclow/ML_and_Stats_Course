...
n_clinic <- 8; n_patient <- 25; n_visit <- 5
clinic_effect  <- rnorm(n_clinic, 0, 6)
patient_effect <- rnorm(n_clinic * n_patient, 0, 8)
bp <- expand_grid(clinic = 1:n_clinic, patient = 1:n_patient, visit = 1:n_visit) |>
  mutate(
    patient_id = interaction(clinic, patient),
    pid_index  = as.integer(patient_id),
    sbp = 135 + clinic_effect[clinic] + patient_effect[pid_index] +
          -1.5 * (visit - 1) + rnorm(n(), 0, 5)
  )
...
### Exercise 1: Spot the clustering (conceptual)
...
### Exercise 2: Fit and interpret a random-intercept model
...
### Exercise 3: Random intercept vs random slope
...
### Exercise 4: GLMM and the marginal/conditional distinction (conceptual)
...
The standard reference for fitting mixed models in R is @bates2015lme4, the paper describing the `lme4` package and its `lmer`/`glmer` functions; @kuznetsova2017lmertest documents the companion `lmerTest` package that supplies p-values, and @brooks2017glmmtmb introduces `glmmTMB` for more flexible variance structures. For a gentle, applied introduction aimed squarely at health researchers, @twisk2013longitudinal ("Applied Longitudinal Data Analysis for Epidemiology") is hard to beat and explains the repeated-measures setting with clinical examples throughout. The deeper statistical treatment of multilevel and hierarchical modelling, including partial pooling and shrinkage, is given by @gelman2007data. For the distinction between marginal (GEE) and conditional (mixed) models --- a recurring source of confusion in clinical papers --- @hubbard2010gee ("To GEE or Not to GEE") is a clear and practical comparison, and the `geepack` package in R implements the GEE approach. Two accessible journal tutorials are worth reading alongside this chapter: @schober2018repeated, in the *Anesthesia & Analgesia* statistics series, reviews summary-statistic, repeated-measures ANOVA, GEE and mixed-model strategies for longitudinal data using published clinical examples, and the brief conceptual series by @merlo2005multilevel in the *Journal of Epidemiology & Community Health* links the statistical idea of clustering and the ICC to multilevel modelling without the heavy mathematics. Readers wanting the Python route should consult the `statsmodels` `MixedLM` documentation, which mirrors the concepts here for linear mixed models.
