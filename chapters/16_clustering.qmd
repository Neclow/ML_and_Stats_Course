...
K-means is the simplest and most widely used clustering algorithm. The "k" is just the number of groups you ask for, and "means" refers to the group averages it keeps recalculating. Given $k$ (the number of clusters), it works as follows:

1. **Initialise**: randomly place $k$ **centroids** in the feature space. A centroid is simply the centre point of a cluster --- think of it as the "average patient" of that group.
...
4. **Repeat** steps 2--3 until assignments stop changing (**convergence** --- the point where another round would not move anyone).
...
%%| echo: false
%%| eval: true
%%| label: fig-kmeans-loop
%%| fig-cap: "The k-means loop: assign patients to the nearest centre, then move each centre to the average of its members, and repeat until nobody moves."
flowchart TD
    A["Place k centres<br/>at random"] --> B["Assign each patient<br/>to nearest centre"]
    B --> C["Move each centre to the<br/>average of its members"]
    C --> D{"Did any patient<br/>change group?"}
    D -->|Yes| B
    D -->|No| E["Done: clusters<br/>have settled"]
```

K-means minimises the **within-cluster sum of squares (WCSS)** --- a single number measuring how tightly packed the groups are. It is the total squared distance from each patient to their assigned centroid.
...
The most important decision in K-means is the number of clusters $k$. You have to tell the algorithm how many groups to look for, but the data does not come with that answer. There is no single correct value, but several heuristics help.

**Elbow method.** Plot WCSS against $k$. As $k$ increases, WCSS always falls (more clusters means every patient sits closer to a centre). The trick is to look for an "elbow" --- the point where the curve stops dropping steeply and flattens out, much like the bend in a bent arm. The elbow marks where adding more clusters buys you little extra tightness, so it is a sensible stopping point.

**Silhouette scores.** For each patient, the silhouette score asks: am I much closer to my own group than to the nearest rival group? Scores run from --1 (the patient looks misplaced --- closer to another group) through 0 (sitting on a boundary) to +1 (comfortably in the right group). Average the score over all patients to get a single quality number. Higher is better; choose the $k$ that maximises it.

**Gap statistic.** Compares the tightness (WCSS) of your real data to the tightness you would get from structureless random data spread evenly across the same space (the "reference null"). The "gap" is largest at the $k$ where your data shows more grouping than random noise would --- in other words, where the structure is most likely to be real.
...
%%| echo: false
%%| eval: true
%%| label: fig-choosing-k
%%| fig-cap: "Choosing the number of clusters: combine the statistical heuristics, then sanity-check against clinical knowledge."
flowchart TD
    A["How many clusters?"] --> B["Elbow: where does<br/>WCSS stop dropping?"]
    A --> C["Silhouette: which k<br/>scores highest?"]
    A --> D["Gap statistic: where does<br/>structure beat random?"]
    B --> E{"Do they<br/>agree?"}
    C --> E
    D --> E
    E -->|Yes| F["Confirm it makes<br/>clinical sense"]
    E -->|No| G["Use clinical judgement;<br/>try a small range of k"]
```
...
Unlike K-means, hierarchical clustering does not make you commit to a number of clusters up front. The **agglomerative** (meaning "bottom-up", or merging) approach starts with every patient alone in their own cluster, then repeatedly fuses the two closest clusters --- step by step --- until everyone has been merged into a single group.

The result is a **dendrogram** --- a tree-like diagram (think of a family tree drawn upside down) that records the whole merging history. You decide on the number of clusters afterwards by "cutting" the tree at a chosen height: a low cut leaves many small groups, a high cut leaves a few large ones.
...
The merging rule hinges on one question: once clusters contain several patients each, how do you measure the distance *between two clusters*? The answer is the **linkage method** --- the recipe for turning many patient-to-patient distances into a single cluster-to-cluster distance.

**Single linkage.** Distance = the *closest* pair of patients across the two clusters. Because two groups count as near if they have even one near-neighbour, this tends to produce long, straggly, chain-like clusters and is easily thrown off by outliers and noise.

**Complete linkage.** Distance = the *farthest* pair of patients. Groups merge only when even their most distant members are reasonably close, so this yields tight, roughly equal-sized clusters --- but a single extreme patient can distort it.
...
**Ward's method.** At each step, merges the pair of clusters that increases total within-cluster spread (WCSS) the least --- in other words, it always makes the "tidiest" merge available. It tends to produce compact, roughly spherical clusters of similar size. **This is usually the best default choice for clinical data.**
...
K-means assumes clusters are spherical and roughly equal in size. In clinical data, clusters may be irregularly shaped, overlap, or contain noise (outlier patients who do not belong to any group). **DBSCAN** (which stands for Density-Based Spatial Clustering of Applications with Noise) addresses these limitations. The name is a mouthful, but the idea is simple: it finds groups by looking for crowded regions of patients.

### How It Works

DBSCAN defines clusters as dense (crowded) regions of patients separated by sparse (empty) regions --- rather like spotting towns on a night-time satellite photo by their clusters of lights. Two settings control its behaviour:

- **eps ($\varepsilon$)**: how close two patients must be to count as neighbours --- the radius of the neighbourhood drawn around each patient.
- **min_samples**: how many neighbours a patient needs within that radius before the surrounding area counts as "crowded" (dense).

The algorithm then sorts every patient into one of three types:
...
**Calinski-Harabasz index.** Compares the spread *between* clusters to the spread *within* clusters (a ratio; higher is better). A high value means the groups are tight inside and far apart from each other --- exactly what a good clustering looks like.
...
dat %>%
  ...
  knitr::kable(
    digits = 2,
    caption = "Cluster profiles: mean of each variable (original scale) within each cluster."
  )
```
