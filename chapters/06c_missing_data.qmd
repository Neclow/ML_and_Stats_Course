...
n <- 800
full <- tibble(
  age = rnorm(n, 60, 12), bmi = rnorm(n, 28, 5),
...
### Exercise 2: Quantify the cost of complete-case analysis
...
### Exercise 3: Multiple imputation end to end
...
### Exercise 4: MNAR sensitivity analysis (conceptual + code)
...
The definitive modern reference is @vanbuuren2018flexible, *Flexible Imputation of Missing Data* (2nd edition), written by the author of the `mice` package and freely available online; it is the natural next stop for any health researcher doing imputation seriously. The chained-equations algorithm itself, and the `mice` software, are described in @vanbuuren2011mice. The foundational theory --- multiple imputation and the pooling rules that bear his name --- is due to @rubin1987multiple. For the practical typology of MCAR/MAR/MNAR and likelihood-based handling, @little2019statistical (*Statistical Analysis with Missing Data*, 3rd edition) is the standard text. A concise, clinically oriented introduction is @sterne2009mi, the widely cited *BMJ* guidance on multiple imputation and its pitfalls, which is excellent for a first read. For even shorter journal explainers pitched at clinicians, the *JAMA Guide to Statistics and Methods* pair @newgard2015missing (handling missing data) and @li2015multiple (multiple imputation) are ideal first reads, and @pedersen2017missing offers a clear epidemiological tour of MCAR/MAR/MNAR and multiple imputation in observational research. Recent methodological work, such as @curnow2024auxiliary, refines the advice on auxiliary variables and the risks of MNAR. In Python, scikit-learn's `IterativeImputer` provides MICE-style imputation, with `miceforest` and the `statsmodels` `MICE` module as fuller-featured alternatives.
