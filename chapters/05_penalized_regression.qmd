...
coef_tab <- coef_tab[coef_tab[["lambda.1se"]] != 0 | coef_tab[["lambda.min"]] != 0, ]
knitr::kable(
  coef_tab,
...
deviance_tab <- data.frame(
  ...
  `CV deviance` = round(c(min(cv_ridge$cvm), min(cv_enet$cvm), min(cv_lasso$cvm)), 4),
  check.names  = FALSE
)
knitr::kable(
  deviance_tab,
...
knitr::kable(
  data.frame(Variable = rownames(sel), Coefficient = round(sel[, 1], 3),
             row.names = NULL),
