...
flowchart TB
    ...
    subgraph LOOP["Repeat ~200 times"]
        B["Draw a bootstrap sample<br/>(resample patients with replacement)"]
        B --> C["Refit the model on the bootstrap sample"]
        C --> D["Score it on the bootstrap sample<br/>(looks good)"]
        C --> E["Score the SAME model on the original data<br/>(more realistic)"]
        D --> F["Optimism = bootstrap score - original score"]
        E --> F
    end
    F --> G["Average the optimism"]
