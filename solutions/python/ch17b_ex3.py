...
trt = np.random.binomial(1, 0.5, n)              # randomised: no confounding
# TRUE effect varies with age (effect modification): stronger when younger.
eff = -1.2 + 0.03 * (age - 65)                   # per-patient log-odds effect
death = np.random.binomial(1, 1 / (1 + np.exp(-(-0.5 + 0.04 * (age - 65) + eff * trt))))
d = pd.DataFrame(dict(age=age, trt=trt, death=death))

# Correct (interaction) model, then standardise:
m_int = smf.glm("death ~ trt * age", data=d, family=sm.families.Binomial()).fit()
p1 = m_int.predict(d.assign(trt=1)).mean()
p0 = m_int.predict(d.assign(trt=0)).mean()
print(f"Standardised risk difference (interaction model): {p1 - p0:+.4f}")

# True marginal risk difference for comparison (we know the DGP):
tp1 = (1 / (1 + np.exp(-(-0.5 + 0.04 * (age - 65) + eff * 1)))).mean()
tp0 = (1 / (1 + np.exp(-(-0.5 + 0.04 * (age - 65) + eff * 0)))).mean()
print(f"TRUE marginal risk difference:                    {tp1 - tp0:+.4f}")

# The no-interaction coefficient is a single number that ignores the variation:
m_noint = smf.logit("death ~ trt + age", data=d).fit(disp=0)
print(f"No-interaction treat coefficient (log-odds):      {m_noint.params['trt']:+.4f}")
print("-> The standardised interaction model matches the truth; the single")
print("   no-interaction coefficient is on a different scale and cannot express")
print("   an effect that varies by age.")
